...
public:
  ...
  bool loadConfigs(const string &filePath, const PartitionTable &partitionTable,
                   vector<string> *ipToWatch, CpuCfg *couConfig, vector<DiskCfg> *disksToMonitor,
                   int *check_if_idle_every, int *stop_monitoring_for, int *reset_monitoring_after,
                   int *suspend_after);
  ...
  // instead (if provided)
  template <typename T>
  bool loockupFieldInCfgFile(const Setting &scope, const string &fieldName, T &output,
  ...
  void parseMultiChoiceArgs(const string &input, vector<string> *output,
  ...
  void parseMultiChoiceSupportingAll(const string &input, vector<string> *output,
